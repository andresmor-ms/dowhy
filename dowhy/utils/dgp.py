...
class DataGeneratingProcess:
    ...
    def __init__(self, **kwargs):
        ...
        if self.treatment_is_binary:
            self.percentile = kwargs.pop(
                "percentile", DataGeneratingProcess.DEFAULT_PERCENTILE
            )
        ...
        if deterministic:
            ...
            if self.percentile >= 0.5:
                mask = np.random.binomial(
                    1,
                    (1 - self.percentile) * 2,
                    len(binary_treat_value[binary_treat_value == 1]),
                )
                binary_treat_value[binary_treat_value == 1] = (
                    mask * binary_treat_value[binary_treat_value == 1]
                )
            else:
                mask = np.random.binomial(
                    1,
                    1 - self.percentile * 2,
                    len(binary_treat_value[binary_treat_value == 0]),
                )
                binary_treat_value[binary_treat_value == 0] = (
                    mask + binary_treat_value[binary_treat_value == 0]
                )
