...
class RandomNeuralNetwork(DataGeneratingProcess):
    ...
    def __init__(self, **kwargs):
        ...
        self.nn["confounder=>outcome"] = MLPRegressor(
            random_state=self.random_state,
            hidden_layer_sizes=self.arch["confounder=>outcome"],
        )
        self.nn["effect_modifier=>outcome"] = MLPRegressor(
            random_state=self.random_state,
            hidden_layer_sizes=self.arch["effect_modifier=>outcome"],
        )
        self.nn["treatment=>outcome"] = MLPRegressor(
            random_state=self.random_state,
            hidden_layer_sizes=self.arch["treatment=>outcome"],
        )
    ...
    def generate_data(self, sample_size):
        ...
        outcome = (
            self.nn["confounder=>outcome"].predict(
                np.hstack((confounder, effect_modifier))
            )
            + self.nn["effect_modifier=>outcome"].predict(effect_modifier)
            + self.nn["treatment=>outcome"].predict(
                np.hstack((confounder, effect_modifier, treatment))
            )
        )
        y_control = (
            self.nn["confounder=>outcome"].predict(
            ...
            + self.nn["effect_modifier=>outcome"].predict(effect_modifier)
            + self.nn["treatment=>outcome"].predict(
                np.hstack((confounder, effect_modifier, control_value))
            )
        )
        y_treatment = (
            self.nn["confounder=>outcome"].predict(
            ...
            + self.nn["effect_modifier=>outcome"].predict(effect_modifier)
            + self.nn["treatment=>outcome"].predict(
                np.hstack((confounder, effect_modifier, treatment_value))
            )
        )
    ...
    def generation_process(self):
        X = np.random.randn(
            RandomNeuralNetwork.TRAINING_SAMPLE_SIZE, len(self.confounder)
        )
        y = np.random.randn(
            RandomNeuralNetwork.TRAINING_SAMPLE_SIZE, len(self.treatment)
        )
        self.nn["confounder=>treatment"].fit(X, y)

        X = np.random.randn(
            RandomNeuralNetwork.TRAINING_SAMPLE_SIZE,
            len(self.confounder) + len(self.effect_modifier),
        )
        y = np.random.randn(RandomNeuralNetwork.TRAINING_SAMPLE_SIZE, len(self.outcome))
        self.nn["confounder=>outcome"].fit(X, y)

        X = np.random.randn(
            RandomNeuralNetwork.TRAINING_SAMPLE_SIZE, len(self.effect_modifier)
        )
        y = np.random.randn(RandomNeuralNetwork.TRAINING_SAMPLE_SIZE, len(self.outcome))
