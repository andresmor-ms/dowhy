...
class LinearDataGeneratingProcess(DataGeneratingProcess):
    ...
    def generate_data(self, sample_size):
        ...
        effect_modifier = np.zeros(
            (sample_size, len(self.effect_modifier))
        )  # random.randn
    ...
    def generation_process(self):
        self.weights["confounder=>treatment"] = self.generate_weights(
            (len(self.confounder), len(self.treatment))
        )
        ...
        self.weights["confounder=>outcome"] = self.generate_weights(
            (len(self.confounder), len(self.outcome))
        )
        ...
        self.weights["effect_modifier=>outcome"] = self.generate_weights(
            (len(self.effect_modifier), len(self.outcome))
        )
        self.weights["treatment=>outcome"] = self.generate_weights(
            (len(self.treatment), len(self.outcome))
        )
