...
def compute_ci(r=None, nx=None, ny=None, confidence=0.95):
    ...
    ci_z = np.array(
        [z - crit * se, z + crit * se]
    )  # CI = point estimator ± critical value*error
...
def partial_corr(data=None, x=None, y=None, z=None, method="pearson"):
    ...
    if method == "spearman":
        V = data.rank(
            na_option="keep"
        ).cov()  # Change data to rank for spearman correlation
    else:
    ...
    D = np.diag(
        np.sqrt(1 / Vi_diag)
    )  # Storing Standard Deviations from diagonal of inverse covariance matrix
    pcor = -1 * (D @ Vi @ D)
    r = pcor[0, 1]
    ...
    pval = 2 * t.sf(
        np.abs(tval), dof
    )  # Calculate p-value corresponding to the test statistic and degree of freedom
...
def conditional_MI(data=None, x=None, y=None, z=None):
    ...
    Hxz = entropy(
        map(lambda x: "%s/%s" % x, zip(X, Z))
    )  # Finding Joint entropy of X and Z
    Hyz = entropy(
        map(lambda x: "%s/%s" % x, zip(Y, Z))
    )  # Finding Joint entropy of Y and Z
    Hz = entropy(Z)  # Finding Entropy of Z
    Hxyz = entropy(
        map(lambda x: "%s/%s/%s" % x, zip(X, Y, Z))
