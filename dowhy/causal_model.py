...
class CausalModel:
    ...
    def estimate_effect(
        ...
        if method_name is None:
            ...
            if num_components > 2:
                ...
                if estimator_package == "dowhy":  # For updated dowhy methods
                    ...
                    method_params[
                        third_party_estimator_package + "_methodname"
                    ] = estimator_name
        ...
        if identified_estimand.no_directed_path:
            self.logger.warning(
                "No directed path from {0} to {1}.".format(
                    self._treatment, self._outcome
                )
            )
        ...
        # Check if estimator's target estimand is identified
            ...
            if fit_estimator:
                if method_params is not None and (
                    num_components <= 2 or estimator_package == "dowhy"
                ):
                ...
                self.causal_estimator.update_input(
                    treatment_value, control_value, target_units
                )
    ...
    def refute_estimate(self, estimand, estimate, method_name=None, **kwargs):
        ...
        refuter = refuter_class(
            self._data, identified_estimand=estimand, estimate=estimate, **kwargs
        )
    ...
    def summary(self, print_to_stdout=False):
        ...
        """
        summary_text = (
            "Model to find the causal effect of treatment {0} on outcome {1}".format(
                self._treatment, self._outcome
            )
        )
    ...
    def refute_graph(self, k=1, independence_test=None, independence_constraints=None):
        ...
        if independence_constraints is None:
            ...
            for (
            ...
            ):  # Iterate over the unique 2-sized sets [x,y]
                i = combination[0]
                j = combination[1]
                a = all_nodes[i]
                b = all_nodes[j]
                ...
                for k_list in k_sized_lists:
                    if (
                        self._graph.check_dseparation([str(a)], [str(b)], k_list)
                        == True
                    ):
