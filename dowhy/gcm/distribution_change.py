...
def mechanism_change_test(
    ...
    num_samples_for_testing = min(
        target_original_data.shape[0], target_new_data.shape[0]
    )
    ...
    original_indices = np.random.choice(
        target_original_data.shape[0], num_samples_for_testing, replace=False
    )
    new_indices = np.random.choice(
        target_new_data.shape[0], num_samples_for_testing, replace=False
    )

    joint_target_samples = np.vstack(
        [
            shape_into_2d(target_original_data[original_indices]),
    ...
    if parents_original_data is None:
        ...
        joint_parent_data = np.vstack(
            [
                shape_into_2d(parents_original_data[original_indices]),
                shape_into_2d(parents_new_data[new_indices]),
            ]
        )

        return independence_test(
            joint_target_samples, data_set_indices, joint_parent_data
        )
...
def distribution_change(
    ...
    **Research Paper**:
    ...
    """
    causal_graph_old = graph_factory(
...
def distribution_change_of_graphs(
    ...
    return __estimate_marginal_distribution_change(
        ProbabilisticCausalModel(
            node_connected_subgraph_view(causal_model_old.graph, target_node)
        ),
        ProbabilisticCausalModel(
            node_connected_subgraph_view(causal_model_new.graph, target_node)
        ),
        target_node,
        num_samples,
        difference_estimation_func,
...
def _fit_accounting_for_mechanism_change(
    ...
    old_data: pd.DataFrame,
    new_data: pd.DataFrame,
    mechanism_change_func: Callable[
        [np.ndarray, np.ndarray, Optional[np.ndarray], Optional[np.ndarray]], float
    ],
...
def __estimate_marginal_distribution_change(
    ...
    old_causal_models = [
        causal_model_old.causal_mechanism(x)
        for x in sorted(causal_model_old.graph.nodes)
    ]
    new_causal_models = [
        causal_model_new.causal_mechanism(x)
        for x in sorted(causal_model_new.graph.nodes)
    ]

    target_samples_old = draw_samples(causal_model_old, num_samples)[
        target_node
    ].to_numpy()
    ...
    def attribution_set_function(subset):
        ...
        for node in causal_model.graph.nodes:
            causal_model.graph.nodes[node][
                PARENTS_DURING_FIT
            ] = get_ordered_predecessors(causal_model.graph, node)

        target_samples_new = draw_samples(causal_model, num_samples)[
            target_node
        ].to_numpy()

        return difference_estimation_func(target_samples_old, target_samples_new)

    attributions = estimate_shapley_values(
        attribution_set_function, len(old_causal_models), shapley_config
    )

    return {
        x: attributions[i] for i, x in enumerate(sorted(causal_model_old.graph.nodes))
    }
...
def estimate_distribution_change_scores(
    ...
    for node in tqdm(
        ...
        desc="Estimating mechanism change anomaly scores",
        position=0,
        leave=True,
    ):
        if mechanism_changed_for_node[node]:
            if is_root_node(causal_model.graph, node):
                results[node] = difference_estimation_func(
                    original_data[node].to_numpy(), new_data[node].to_numpy()
                )
            else:
...
def _check_significant_mechanism_change(
    ...
    for node in graph.nodes:
        if is_root_node(graph, node):
            ...
            parents_org_data = old_data[
                get_ordered_predecessors(graph, node)
            ].to_numpy()
            parents_new_data = new_data[
                get_ordered_predecessors(graph, node)
            ].to_numpy()
    ...
    if fdr_control_method is None:
        ...
        successes = multipletests(
            all_p_values, significance_level, method=fdr_control_method
        )[0]
...
def _estimate_distribution_change_score(
    ...
    joint_parent_samples = joint_parent_samples[
        np.random.choice(
            joint_parent_samples.shape[0],
            min(joint_parent_samples.shape[0], max_num_evaluation_samples),
            replace=False,
        )
    ]
    ...
    for joint_parent_sample in joint_parent_samples:
        ...
        result += difference_estimation_func(
            causal_model_original.draw_samples(samples),
