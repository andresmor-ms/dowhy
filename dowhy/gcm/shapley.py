...
def estimate_shapley_values(
    ...
    if approximation_method == ShapleyApproximationMethods.EXACT:
        return _estimate_shapley_values_exact(
            set_func=set_func, num_players=num_players, n_jobs=shapley_config.n_jobs
        )
...
def _estimate_shapley_values_exact(
    ...
    all_subsets = [
        tuple(subset) for subset in itertools.product([0, 1], repeat=num_players)
    ]
    ...
    subsets_missing_one_player = np.array(
        list(itertools.product([0, 1], repeat=num_players - 1))
    )
    for player_index in range(num_players):
        ...
        for i in range(len(subsets_with_player)):
            ...
            if subset_length not in subset_weight_cache:
                subset_weight_cache[subset_length] = compute_subset_weight(
                    subset_length
                )

            weighted_diff = subset_weight_cache[subset_length] * (
                subset_to_result_map[subsets_with_player[i]]
...
def _approximate_shapley_values_via_least_squares_regression(
    ...
    if not use_subset_approximation:
        all_subsets, weights = _create_subsets_and_weights_exact(
            num_players, full_and_empty_subset_weight
        )
    else:
    ...
    return (
        LinearRegression()
        .fit(all_subsets, np.array(set_function_results), sample_weight=weights)
        .coef_
    )
...
def _approximate_shapley_values_via_permutation_sampling(
    ...
    full_subset_result, empty_subset_result = _estimate_full_and_emtpy_subset_results(
        set_func, num_players
    )
    ...
    with Parallel(n_jobs=n_jobs) as parallel:
        evaluated_subsets = _evaluate_set_function(
            set_func, subsets_to_evaluate, parallel
        )
...
def _approximate_shapley_values_via_early_stopping(
    ...
    with Parallel(n_jobs=n_jobs) as parallel:
        ...
        while True:
            ...
            # could be [3,1,4,2].
            permutations = [
                np.random.choice(num_players, num_players, replace=False)
            ...
            for permutation in permutations:
                ...
                subsets_to_evaluate.update(
                    [
                        subset_tuple
                        for subset_tuple in _create_index_order_and_subset_tuples(
                            permutation
                        )
            ...
            evaluated_subsets.update(
                _evaluate_set_function(set_func, subsets_to_evaluate, parallel, False)
            )
...
def _create_subsets_and_weights_exact(
    num_players: int, high_weight: float
        ...
        all_subsets.extend(
            np.array(
                [
                    ...
                    for combs in itertools.combinations(range(num_players), i)
                ]
            )
        )
            ...
            np.array(
                [
                    ...
                    for combs in itertools.combinations(
                        range(num_players), num_players - i
                    )
                ]
            )
        )
...
def _create_subsets_and_weights_approximation(
    ...
    probabilities_of_subset_length = probabilities_of_subset_length / np.sum(
        probabilities_of_subset_length
    )
...
def _convert_list_of_indices_to_binary_vector_as_tuple(
    list_of_indices: List[int], num_players: int
) -> Tuple[int]:
...
def _evaluate_set_function(
    ...
    def parallel_job(
        input_subset: Tuple[int], parallel_random_seed: int
    ...
    random_seeds = np.random.randint(
        np.iinfo(np.int32).max, size=len(evaluation_subsets)
    )
...
def _estimate_full_and_emtpy_subset_results(
    ...
    return set_func(np.ones(num_players, dtype=np.int)), set_func(
        np.zeros(num_players, dtype=np.int)
    )
...
def _estimate_shapley_values_of_permutation(
    ...
    for n in range(len(permutation) - 1):
        ...
        current_result = evaluated_subsets[
            _convert_list_of_indices_to_binary_vector_as_tuple(
                current_variable_set, len(permutation)
            )
        ]
