...
def fit_causal_model_of_target(
    ...
    if is_root_node(causal_model.graph, target_node):
        causal_model.causal_mechanism(target_node).fit(
            X=training_data[target_node].to_numpy()
        )
    else:
        causal_model.causal_mechanism(target_node).fit(
            X=training_data[
    ...
    causal_model.graph.nodes[target_node][
        PARENTS_DURING_FIT
    ] = get_ordered_predecessors(causal_model.graph, target_node)


def draw_samples(
    causal_model: ProbabilisticCausalModel, num_samples: int
) -> pd.DataFrame:
    ...
    for node in nx.topological_sort(causal_model.graph):
        if is_root_node(causal_model.graph, node):
            ...
            drawn_samples[node] = causal_model.causal_mechanism(node).draw_samples(
                column_stack_selected_numpy_arrays(
                    drawn_samples, get_ordered_predecessors(causal_model.graph, node)
                )
            )
