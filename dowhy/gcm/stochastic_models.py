...
_DISCRETE_DISTRIBUTIONS = [
    getattr(scipy.stats, d)
    for d in dir(scipy.stats)
    if isinstance(getattr(scipy.stats, d), scipy.stats.rv_discrete)
]
...
class ScipyDistribution(StochasticModel):
    ...
    def draw_samples(self, num_samples: int) -> np.ndarray:
        ...
        return shape_into_2d(
            self._distribution.rvs(size=num_samples, **self.parameters)
        )
    ...
    def clone(self):
        if self._fixed_parameters:
            return ScipyDistribution(
                scipy_distribution=self._distribution, **self._parameters
            )
        else:
    ...
    def find_suitable_continuous_distribution(
        ...
        for distribution in _CONTINUOUS_DISTRIBUTIONS.values():
            ...
            with warnings.catch_warnings():
                ...
                generated_samples = distribution.rvs(
                    size=distribution_samples.shape[0], loc=loc, scale=scale, *arg
                )
                ...
                divergence = estimate_kl_divergence_continuous(
                    distribution_samples, generated_samples
                )
    ...
    def map_scipy_distribution_parameters_to_names(
        ...
        if scipy_distribution.shapes:
            parameter_list = [
                name.strip() for name in scipy_distribution.shapes.split(",")
            ]
        else:
            parameter_list = []
...
class EmpiricalDistribution(StochasticModel):
    ...
    def draw_samples(self, num_samples: int) -> np.ndarray:
        ...
        return self.data[
            np.random.choice(self.data.shape[0], size=num_samples, replace=True), :
        ]
...
class BayesianGaussianMixtureDistribution(StochasticModel):
    ...
    def fit(self, X: np.ndarray) -> None:
        ...
        self.__gmm_model = BayesianGaussianMixture(
            n_components=BayesianGaussianMixtureDistribution.__get_optimal_number_of_components(
                X
            ),
            max_iter=1000,
        ).fit(X)
