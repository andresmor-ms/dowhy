...
def approximate_rbf_kernel_features(
    ...
    return Nystroem(
        kernel="rbf", gamma=precision, n_components=num_random_components
    ).fit_transform(X)


def approximate_delta_kernel_features(
    X: np.ndarray, num_random_components: int
) -> np.ndarray:
    ...
    result = Nystroem(
        kernel=delta_function, n_components=num_random_components
...
def auto_create_list_of_kernels(
    X: np.ndarray,
...
def auto_create_list_of_kernel_approximations(
    X: np.ndarray,
