...
def kernel_based(
    ...
    """
    bootstrap_n_jobs = (
        config.default_n_jobs if bootstrap_n_jobs is None else bootstrap_n_jobs
    )
...
def _rit(
    ...
    def evaluate_rit_on_samples(parallel_random_seed: int):
        ...
        if X.shape[0] > num_max_samples_per_run:
            random_indices = np.random.choice(
                X.shape[0], num_max_samples_per_run, replace=False
            )
        ...
        return 1 - (
            np.sum(
                _estimate_rit_statistic(random_features_x, random_features_y)
                > permutation_results_of_statistic
            )
    ...
    p_values = Parallel(n_jobs=n_jobs)(
        delayed(evaluate_rit_on_samples)(random_seeds[i]) for i in range(num_runs)
    )
...
def _rcit(
    ...
    def parallel_job(parallel_random_seed: int):
        ...
        inverse_cov_zz = scipy.linalg.cho_solve(
            scipy.linalg.cho_factor(
                cov_zz + np.eye(cov_zz.shape[0]) * 10**-10, lower=True
            ),
        ...
        return 1 - (
            np.sum(
                _estimate_rit_statistic(residual_x, residual_y)
                > permutation_results_of_statistic
            )
            / len(permutation_results_of_statistic)
        )
    ...
    random_seeds = np.random.randint(np.iinfo(np.int32).max, size=num_runs)
    p_values = Parallel(n_jobs=n_jobs)(
        delayed(parallel_job)(random_seeds[i]) for i in range(num_runs)
    )
