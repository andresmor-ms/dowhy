        ...
        if is_root_node(causal_model.graph, node):
            ...
            if isinstance(prediction_model, ClassificationModel):
                ...
                causal_model.set_causal_mechanism(
                    node, AdditiveNoiseModel(prediction_model)
                )
...
def has_linear_relationship(
    X: np.ndarray, Y: np.ndarray, max_num_samples: int = 3000
) -> bool:
    ...
    if target_is_categorical:
        all_classes, indices, counts = np.unique(
            Y, return_counts=True, return_index=True
        )
        ...
        x_train, x_test, y_train, y_test = train_test_split(
            X,
            Y,
            train_size=num_trainings_samples,
    ...
    if target_is_categorical:
        ...
        return np.mean(
            (y_test - shape_into_2d(linear_mdl.predict(x_test))) ** 2
...
def find_best_model(
    ...
    is_classification_problem = isinstance(
        prediction_model_factories[0](), ClassificationModel
    )
    ...
    def estimate_average_score(
        prediction_model_factory: Callable[[], PredictionModel], random_seed: int
    ) -> float:
        ...
        with warnings.catch_warnings():
            ...
            for train_indices, test_indices in kfolds:
                ...
                model_instance.fit(
                    X[train_indices[:max_samples_per_split]],
                    Y[train_indices[:max_samples_per_split]],
                )
    ...
    random_seeds = np.random.randint(
        np.iinfo(np.int32).max, size=len(prediction_model_factories)
    )
    average_metric_scores = Parallel(n_jobs=n_jobs)(
        delayed(estimate_average_score)(prediction_model_factory, random_seed)
        for prediction_model_factory, random_seed in zip(
            prediction_model_factories, random_seeds
        )
    )

    return sorted(
        zip(prediction_model_factories, average_metric_scores), key=lambda x: x[1]
    )[0][0]
