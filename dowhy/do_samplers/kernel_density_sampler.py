    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            ...
            or max(
                ...
                len(
                    self._outcome_names + self._target_estimand.get_backdoor_variables()
                ),
            )
            >= 3
        ):
...
class KernelSampler(object):
    ...
    def sample_point(self, x_z):
        y_bw = (
            1.06
            * self._data[self._outcome_names].std()
            * (self._data[self._outcome_names].count()) ** (-1.0 / 5.0)
        )
        n = 5 * np.ceil(
        ...
        cum_ranges = [
            np.linspace(
                self.outcome_lower_support[i], self.outcome_upper_support[i], n[i]
            )
        ...
        points = (
            np.array(res).reshape(len(self._outcome_names), np.int(n.cumprod()[-1])).T
        )
        ...
        points = np.vstack(
            [
                [self.outcome_lower_support - 3.0 * y_bw],
                points,
                [self.outcome_upper_support + 3.0 * y_bw],
            ]
        )
