...
class McmcSampler(DoSampler):
    def __init__(
        ...
        **kwargs,
        ...
        _, self.fit_trace = self.fit_causal_model(
            g_fit, self._data, self._variable_types
        )
    ...
    def apply_parents(self, g):
        for node in nx.topological_sort(g):
            if not g.nodes()[node].get("parent_names"):
                g.nodes()[node]["parent_names"] = [
                    parent for parent, _ in g.in_edges(node)
                ]
        return g
        ...
        for node in nx.topological_sort(g):
            ...
            if parent_names:
                if not initialization_trace:
                    sd = np.array(
                        [df[node].std()]
                        + (df[node].std() / df[parent_names].std()).tolist()
                    )
                    mu = np.array(
                        [df[node].std()]
                        + (df[node].std() / df[parent_names].std()).tolist()
                ...
                g.nodes()[node]["sd"] = pm.Exponential(
                    "{}_sd".format(node), lam=node_sd
                )
        return g
    ...
    def build_bayesian_network(self, g, df):
        for node in nx.topological_sort(g):
            if g.nodes()[node]["parent_names"]:
                ...
                mu += pm.math.dot(
                    df[g.nodes()[node]["parent_names"]],
                    g.nodes()[node]["parameters"][1:],
                )
                if g.nodes()[node]["variable_type"] == "c":
                    sd = g.nodes()[node]["sd"]
                    g.nodes()[node]["variable"] = pm.Normal(
                        "{}".format(node), mu=mu, sd=sd, observed=df[node]
                    )
                elif g.nodes()[node]["variable_type"] == "b":
                    g.nodes()[node]["variable"] = pm.Bernoulli(
                        "{}".format(node), logit_p=mu, observed=df[node]
                    )
                else:
                    raise Exception(
                        "Unrecognized variable type: {}".format(
                            g.nodes()[node]["variable_type"]
                        )
                    )
        return g
    ...
    def fit_causal_model(self, g, df, data_types, initialization_trace=None):
        if nx.is_directed_acyclic_graph(g):
            with pm.Model() as model:
                ...
                g = self.apply_parameters(
                    g, df, initialization_trace=initialization_trace
                )
