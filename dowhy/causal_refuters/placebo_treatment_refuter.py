...
class PlaceboTreatmentRefuter(CausalRefuter):
    ...
    def refute_estimate(self):
        ...
        treatment_name = self._treatment_name[
            0
        ]  # Extract the name of the treatment variable
        ...
        for index in range(self._num_simulations):
            ...
            if self._placebo_type == "permute":
                ...
                new_treatment = (
                    self._data[self._treatment_name].iloc[permuted_idx].values
                )
                if self._target_estimand.identifier_method.startswith("iv"):
                    new_instruments_values = (
                        self._data[self._estimate.estimator.estimating_instrument_names]
                        .iloc[permuted_idx]
                        .values
                    )
                    new_instruments_df = pd.DataFrame(
                        ...
                        columns=[
                            "placebo_" + s
                            for s in self._data[
                                self._estimate.estimator.estimating_instrument_names
                            ].columns
                        ],
                    )
            else:
                if "float" in type_dict[treatment_name].name:
                    ...
                    self.logger.info(
                        "Using a Discrete Uniform Distribution with the following categories:{}".format(
                            categories
                        )
                    )
