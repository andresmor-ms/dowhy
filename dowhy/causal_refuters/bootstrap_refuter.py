...
class BootstrapRefuter(CausalRefuter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._chosen_variables is None:
            ...
            self.logger.info(
                "INFO: The chosen variables are: " + ",".join(self._chosen_variables)
            )

        if self._probability_of_change is None:
            if self._noise > 1:
                self.logger.error(
                    "Error in using noise:{} for Binary Flip. The value is greater than 1".format(
                        self._noise
                    )
                )
                ...
                for variable in self._chosen_variables:
                    ...
                    if ("float" or "int") in new_data[variable].dtype.name:
                        ...
                        new_data[variable] += np.random.normal(
                            loc=0.0,
                            scale=self._noise * scaling_factor,
                        ...
                        new_data[variable] = np.where(
                            probs < self._probability_of_change,
                        ...
                        changed_data = new_data[variable].apply(
                            lambda row: list(set(categories) - set([row]))
                        )
                        # Choose one out of the remaining
                        changed_data = changed_data.apply(
                            lambda row: random.choice(row)
                        )
                        new_data[variable] = np.where(
                            probs < self._probability_of_change, changed_data
                        )
