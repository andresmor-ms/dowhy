...
class DummyOutcomeRefuter(CausalRefuter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._transformation_list = kwargs.pop(
            "transformation_list", DummyOutcomeRefuter.DEFAULT_TRANSFORMATION
        )
        self._true_causal_effect = kwargs.pop(
            "true_causal_effect", DummyOutcomeRefuter.DEFAULT_TRUE_CAUSAL_EFFECT
        )
        ...
        self._test_fraction = kwargs.pop(
            "_test_fraction", DummyOutcomeRefuter.DEFAULT_TEST_FRACTION
        )
        self._unobserved_confounder_values = kwargs.pop(
            "unobserved_confounder_values",
            DummyOutcomeRefuter.DEFAULT_NEW_DATA_WITH_UNOBSERVED_CONFOUNDING,
        )
    ...
    def refute_estimate(self):
        ...
        self.logger.info(
            "Refutation over {} simulated datasets".format(self._num_simulations)
        )
        self.logger.info(
            "The transformation passed: {}".format(self._transformation_list)
        )
        ...
        for _ in range(self._num_simulations):
            ...
            if estimator_present == False:
                ...
                for key_train, _ in groups:
                    ...
                    total_set = set(
                        [tuple(line) for line in groups.get_group(key_train).values]
                    )
                    base_validation = pd.DataFrame(
                        list(total_set.difference(train_set)),
                    ...
                    if key_train not in causal_effect_map:
                        # As we currently support only one treatment
                        causal_effect_map[key_train] = self._true_causal_effect(
                            validation_df[self._treatment_name[0]]
                        )
        ...
        for action, func_args in transformation_list:
            if callable(action):
                ...
                estimator = self._estimate_dummy_outcome(
                    action, X_train, outcome_train, **func_args
                )
                outcome_train = estimator(X_train)
                outcome_validation = estimator(X_validation)
            elif action == "noise":
    ...
    def preprocess_data_by_treatment(self):
        ...
        * int
        ...
        """
        assert (
            len(self._treatment_name) == 1
        ), "At present, DoWhy supports a simgle treatment variable"

        if self._unobserved_confounder_values is not None:
            self._data["simulated"] = self._unobserved_confounder_values
            self._chosen_variables.append("simulated")

        treatment_variable_name = self._treatment_name[
            0
        ]  # As we only have a single treatment
        ...
        # We use string arguments to account for both 32 and 64 bit varaibles
            ...
            num_bins = (data.max() - data.min()) / (
                self._bucket_size_scale_factor * std_dev
            )
            ...
            raise ValueError(
                "Passed {}. Expected bool, float, int or categorical.".format(
                    variable_type.name
                )
            )
    ...
    def _get_regressor_object(self, action, **func_args):
        ...
        if action == "linear_regression":
            ...
            raise ValueError(
                "The function: {} is not supported by dowhy at the moment.".format(
                    action
                )
            )
    ...
    def permute(self, outcome, permute_fraction):
        ...
        if permute_fraction == 1:
            ...
            changes = np.where(
                np.random.uniform(0, 1, outcome.shape[0]) <= permute_fraction
            )[
                0
            ...
            raise ValueError(
                "The value of permute_fraction is {}. Which is greater than 1.".format(
                    permute_fraction
                )
            )
