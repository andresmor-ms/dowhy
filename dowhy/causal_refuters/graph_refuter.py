...
class GraphRefuter(CausalRefuter):
    ...
    def __init__(
        ...
        self._false_implications = (
            []
        )  # List containing the implications from the graph which hold false for dataset
        self._true_implications = (
            []
        )  # List containing the implications from the graph which hold true for dataset
        self._results = (
            {}
        )  # A dictionary with key as test set and value as [p-value, test_result]
    ...
    def refute_model(self, independence_constraints):
        ...
        refute = GraphRefutation(
            method_name_continuous=self._method_name_continuous,
            method_name_discrete=self._method_name_discrete,
        )
        ...
        for a, b, c in independence_constraints:
            if (
                a in continuous_columns
                and b in continuous_columns
                and all(node in continuous_columns for node in c)
            ):
                # a, b and c are all continuous variables
                if (
                    self._method_name_continuous is None
                    or self._method_name_continuous == "partial_correlation"
                ):
                ...
                a in discrete_columns
                and b in discrete_columns
                and all(node in discrete_columns for node in c)
            ):
                # a, b and c are all discrete variables
                if (
                    self._method_name_discrete is None
                    or self._method_name_discrete == "conditional_mutual_information"
                ):
                ...
                and all(
                    node in continuous_columns or node in binary_columns for node in c
                )
            ):
            ...
            elif all(node in discrete_columns for node in c) and (
                a in discrete_columns or b in discrete_columns
            ):
                # c is discrete and
