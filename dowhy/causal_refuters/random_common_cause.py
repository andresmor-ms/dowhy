...
class RandomCommonCause(CausalRefuter):
    ...
    def refute_estimate(self):
        ...
        self.logger.info(
            "Refutation over {} simulated datasets, each with a random common cause added".format(
                self._num_simulations
            )
        )

        new_backdoor_variables = self._target_estimand.get_backdoor_variables() + [
            "w_random"
        ]
        ...
        for index in range(self._num_simulations):
            if self._random_state is None:
                ...
                new_data = self._data.assign(
                    w_random=self._random_state.normal(size=num_rows)
                )
