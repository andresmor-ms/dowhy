...
class AddUnobservedCommonCause(CausalRefuter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.effect_on_t = (
            kwargs["confounders_effect_on_treatment"]
            if "confounders_effect_on_treatment" in kwargs
            else "binary_flip"
        )
        self.effect_on_y = (
            kwargs["confounders_effect_on_outcome"]
            if "confounders_effect_on_outcome" in kwargs
            else "linear"
        )
        self.kappa_t = (
            kwargs["effect_strength_on_treatment"]
            if "effect_strength_on_treatment" in kwargs
            else None
        )
        self.kappa_y = (
            kwargs["effect_strength_on_outcome"]
            if "effect_strength_on_outcome" in kwargs
            else None
        )
        self.frac_strength_treatment = (
            kwargs["effect_fraction_on_treatment"]
            if "effect_fraction_on_treatment" in kwargs
            else 1
        )
        self.frac_strength_outcome = (
            kwargs["effect_fraction_on_outcome"]
            if "effect_fraction_on_outcome" in kwargs
            else 1
        )
        self.simulated_method_name = (
            kwargs["simulated_method_name"]
            if "simulated_method_name" in kwargs
            else "linear_based"
        )
        self.plotmethod = (
            kwargs["plotmethod"] if "plotmethod" in kwargs else "colormesh"
        )
    ...
    def infer_default_kappa_t(self, len_kappa_t=10):
        ...
        if len(observed_common_causes_names) > 0:
            ...
            observed_common_causes = pd.get_dummies(
                observed_common_causes, drop_first=True
            )
        else:
            raise ValueError(
        ...
        if self.effect_on_t == "binary_flip":
            ...
            corrcoef_var_t = np.corrcoef(observed_common_causes, t, rowvar=False)[
                -1, :-1
            ]
        ...
        min_coeff, max_coeff = self._compute_min_max_coeff(
            min_coeff, max_coeff, self.frac_strength_treatment
        )
        ...
        self.logger.info(
            "(Min, Max) kappa_t for observed common causes, ({0}, {1})".format(
                min_coeff, max_coeff
            )
        )
    ...
    def infer_default_kappa_y(self, len_kappa_y=10):
        ...
        if self.effect_on_y == "binary_flip":
            ...
            corrcoef_var_y = np.corrcoef(observed_common_causes, y, rowvar=False)[
                -1, :-1
            ]
        ...
        min_coeff, max_coeff = self._compute_min_max_coeff(
            min_coeff, max_coeff, self.frac_strength_outcome
        )
        # By default, return a plot with 10 points
        # consider 10 values of the effect of the unobserved confounder
        step = (max_coeff - min_coeff) / len_kappa_y
        self.logger.info(
            "(Min, Max) kappa_y for observed common causes, ({0}, {1})".format(
                min_coeff, max_coeff
            )
        )
        if np.equal(max_coeff, min_coeff):
    ...
    def refute_estimate(self):
        ...
        if not isinstance(self.kappa_t, (list, np.ndarray)) and not isinstance(
            ...
            new_data = self.include_confounders_effect(
                new_data, self.kappa_t, self.kappa_y
            )
            ...
            if isinstance(self.kappa_t, (list, np.ndarray)) and isinstance(
                ...
                if self.plotmethod == "contour":
                    ...
                    cp = plt.pcolormesh(
                        self.kappa_y, self.kappa_t, results_matrix, shading="nearest"
                    )
    ...
    def include_confounders_effect(self, new_data, kappa_t, kappa_y):
        ...
        if self.effect_on_t == "binary_flip":
            ...
            new_data[self._treatment_name] = new_data[
                self._treatment_name
            ].values + np.ndarray(shape=(num_rows, 1), buffer=confounder_t_effect)
        else:
            raise NotImplementedError(
                "'"
                + self.effect_on_t
        ...
        if self.effect_on_y == "binary_flip":
            ...
            # assuming that the original confounder's effect was positive on both.
            # This is to remove the effect of the original confounder.
            new_data[self._outcome_name] = new_data[
                self._outcome_name
            ].values + np.ndarray(shape=(num_rows, 1), buffer=confounder_y_effect)
        else:
            raise NotImplementedError(
                "'"
                + self.effect_on_y
    ...
    def include_simulated_confounder(self, convergence_threshold=0.1, c_star_max=1000):
        ...
        *c1 and c2 are coefficients to the residuals
        ...
        observed_variables_with_treatment_and_outcome = (
            observed_variables + self._treatment_name + self._outcome_name
        )
        ...
        while index < len(correlation_y_list):
            if (
                correlation_y_list[index] - correlation_y_list[index - 1]
        ...
        final_U = self.generate_confounder_from_residuals(
            c1_final, c2_final, d_y, d_t, X
        )
    ...
    def generate_confounder_from_residuals(self, c1, c2, d_y, d_t, X):
        ...
        for j in range(len(d_t)):
            ...
            U.append(
                np.random.normal(simulated_variable_mean, simulated_variable_stddev, 1)
            )
