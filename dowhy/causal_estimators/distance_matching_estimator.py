        ...
        **kwargs,
        ...
        if self._observed_common_causes_names:
            if self.exact_match_cols is not None:
                self._observed_common_causes_names = [
                    v
                    for v in self._observed_common_causes_names
                    if v not in self.exact_match_cols
                ]
    ...
    def _estimate_effect(self):
        ...
        updated_df = pd.concat(
            [
                ...
                self._data[[self._outcome_name, self._treatment_name[0]]],
            ],
            axis=1,
        )
        if self.exact_match_cols is not None:
            updated_df = pd.concat(
                [updated_df, self._data[self.exact_match_cols]], axis=1
            )
            ...
            if self.exact_match_cols is None:
                ...
                for i in range(numtreatedunits):
                    ...
                    control_outcome = np.mean(
                        control.iloc[indices[i]][self._outcome_name].values
                    )
                ...
                for i in range(numtreatedunits):
                    self.matched_indices_att[treated_df_index[i]] = control.iloc[
                        indices[i]
                    ].index.tolist()
            else:
        ...
        if fit_atc:
            ...
            for i in range(numcontrolunits):
                ...
                treated_outcome = np.mean(
                    treated.iloc[indices[i]][self._outcome_name].values
                )
            ...
            for i in range(numcontrolunits):
                self.matched_indices_atc[control_df_index[i]] = treated.iloc[
                    indices[i]
                ].index.tolist()
