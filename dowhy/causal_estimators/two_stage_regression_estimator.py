...
class TwoStageRegressionEstimator(CausalEstimator):
    ...
    def __init__(
        self, *args, first_stage_model=None, second_stage_model=None, **kwargs
    ):
        """
        ...
        if self._target_estimand.identifier_method == "frontdoor":
            ...
            self._instrumental_variables_names = (
                self._target_estimand.get_instrumental_variables()
            )

            if self._instrumental_variables_names:
                self._instrumental_variables = self._data[
                    self._instrumental_variables_names
                ]
            else:
        ...
        if first_stage_model is not None:
            ...
            self.logger.warning(
                "First stage model not provided. Defaulting to sklearn.linear_model.LinearRegression."
            )
        if second_stage_model is not None:
            ...
            self.logger.warning(
                "Second stage model not provided. Defaulting to backdoor.linear_regression."
            )
    ...
    def _estimate_effect(self):
        ...
        modified_target_estimand.backdoor_variables = (
        ...
        natural_indirect_effect = (
            first_stage_estimate.value * second_stage_estimate.value
        )
        ...
        if self._target_estimand.estimand_type == CausalIdentifier.NONPARAMETRIC_NDE:
            ...
            natural_direct_effect = (
                total_effect_estimate.value - natural_indirect_effect
            )
    ...
    def build_first_stage_features(self):
        ...
        if len(self._observed_common_causes_names) > 0:
            ...
            observed_common_causes_vals = pd.get_dummies(
                observed_common_causes_vals, drop_first=True
            )
        if self._effect_modifier_names:
            effect_modifiers_vals = data_df[self._effect_modifier_names]
            effect_modifiers_vals = pd.get_dummies(
                effect_modifiers_vals, drop_first=True
            )
        if type(treatment_vals) is not np.ndarray:
            treatment_vals = treatment_vals.to_numpy()
        if treatment_vals.shape[0] != data_df.shape[0]:
            raise ValueError(
                "Provided treatment values and dataframe should have the same length."
            )
        # Bulding the feature matrix
        n_samples = treatment_vals.shape[0]
        self.logger.debug(
            "Number of samples" + str(n_samples) + str(len(self._treatment_name))
        )
        treatment_2d = treatment_vals.reshape((n_samples, len(self._treatment_name)))
        if len(self._observed_common_causes_names) > 0:
            features = np.concatenate(
                (treatment_2d, observed_common_causes_vals), axis=1
            )
        else:
        ...
        if self._effect_modifier_names:
            for i in range(treatment_2d.shape[1]):
                ...
                new_features = (
                    curr_treatment[:, np.newaxis] * effect_modifiers_vals.to_numpy()
                )
