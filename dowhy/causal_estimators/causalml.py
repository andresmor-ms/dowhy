...
class Causalml(CausalEstimator):
    ...
    def __init__(self, *args, causalml_methodname, **kwargs):
        ...
        self.logger.debug(
            "The identifier method used {}".format(self.identifier_method)
        )
        ...
        if self._instrumental_variable_names:
            ...
            self._instrumental_variables = pd.get_dummies(
                self._instrumental_variables, drop_first=True
            )
        else:
        ...
        self.logger.debug(
            "Effect Modifiers used:" + ",".join(self._effect_modifier_names)
        )
    ...
    def _estimate_effect(self):
        ...
        treatment_name = self._treatment_name[
            0
        ]  # As we have only one treatment variable
