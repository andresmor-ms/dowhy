...
class InstrumentalVariableEstimator(CausalEstimator):
    ...
    def __init__(self, *args, iv_instrument_name=None, **kwargs):
        ...
        self.logger.debug(
            "Instrumental Variables used:" + ",".join(self.estimating_instrument_names)
        )
    ...
    def _estimate_effect(self):
        if (
            len(self.estimating_instrument_names) == 1
            and len(self._treatment_name) == 1
        ):
            ...
            if instrument_is_binary:
                ...
                x1_z = np.mean(
                    self._treatment[self._treatment_name[0]][instrument == 1]
                )
                x0_z = np.mean(
                    self._treatment[self._treatment_name[0]][instrument == 0]
                )
                num = y1_z - y0_z
                deno = x1_z - x0_z
                iv_est = num / deno
                ...
                deno_xz = np.cov(self._treatment[self._treatment_name[0]], instrument)[
                    0, 1
                ]
    ...
    def construct_symbolic_estimator(self, estimand):
        ...
        sym_effect = spstats.Expectation(sym_outcome_derivative) / sp.stats.Expectation(
