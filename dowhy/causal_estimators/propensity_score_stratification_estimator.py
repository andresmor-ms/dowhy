...
class PropensityScoreStratificationEstimator(PropensityScoreEstimator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        args_dict = kwargs
        args_dict.update(
            {"num_strata": num_strata, "clipping_threshold": clipping_threshold}
        )
        super().__init__(
            *args,
    ...
    def _estimate_effect(self):
        if self.recalculate_propensity_score is True:
            ...
            if self.propensity_score_column not in self._data.columns:
                ...
                self.logger.info(
                    f"Using pre-computed propensity score incolumn {self.propensity_score_column}"
                )
        clipped = None
        ...
        weighted_outcomes.columns = [
            "_".join(x) for x in weighted_outcomes.columns.to_numpy().ravel()
        ]
        treatment_sum_name = self._treatment_name[0] + "_sum"
        control_sum_name = "dbar_sum"

        weighted_outcomes["d_y_mean"] = (
            weighted_outcomes["d_y_sum"] / weighted_outcomes[treatment_sum_name]
        )
        weighted_outcomes["dbar_y_mean"] = (
            weighted_outcomes["dbar_y_sum"] / weighted_outcomes["dbar_sum"]
        )
        weighted_outcomes["effect"] = (
            weighted_outcomes["d_y_mean"] - weighted_outcomes["dbar_y_mean"]
        )
        ...
        if self._target_units == "att":
            ...
            est = (
                weighted_outcomes["effect"] * weighted_outcomes[control_sum_name]
            ).sum() / total_control_population
        elif self._target_units == "ate":
            est = (
                weighted_outcomes["effect"]
                * (
                    weighted_outcomes[control_sum_name]
                    + weighted_outcomes[treatment_sum_name]
                )
    ...
    def _get_strata(self, num_strata, clipping_threshold):
        ...
        self._data["strata"] = (
            (self._data[self.propensity_score_column].rank(ascending=True) / num_rows)
            * num_strata
        ).round(0)
        ...
        self._data["d_y"] = (
            self._data[self._treatment_name[0]] * self._data[self._outcome_name]
        )
        ...
        self.logger.debug(
            "After using clipping_threshold={0}, here are the number of data points in each strata:\n {1}".format(
                clipping_threshold,
                clipped.groupby(["strata", self._treatment_name[0]])[
                    self._outcome_name
                ].count(),
            )
        )
