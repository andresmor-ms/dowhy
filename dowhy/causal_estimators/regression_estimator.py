...
class RegressionEstimator(CausalEstimator):
    ...
    def _build_features(self, treatment_values=None, data_df=None):
        ...
        if data_df is None:
            ...
            treatment_vals = pd.get_dummies(
                data_df[self._treatment_name], drop_first=True
            )
            if len(self._observed_common_causes_names) > 0:
                observed_common_causes_vals = data_df[
                    self._observed_common_causes_names
                ]
        ...
        n_treatment_cols = (
            1 if len(treatment_vals.shape) == 1 else treatment_vals.shape[1]
        )
    ...
    def _do(self, treatment_val, data_df=None):
        ...
        interventional_treatment_values = np.full(
            (data_df.shape[0], len(self._treatment_name)), treatment_val
        )
        ...
        interventional_treatment_2d = pd.concat(
            [
                ...
                pd.DataFrame(
                    ...
                    columns=self._treatment.columns,
                ),
            ],
            axis=0,
        ).astype(self._treatment.dtypes, copy=False)
        interventional_treatment_2d = pd.get_dummies(
            interventional_treatment_2d, drop_first=True
        )
        interventional_treatment_2d = interventional_treatment_2d[
            self._treatment.shape[0] :
        ]

        new_features = self._build_features(
            treatment_values=interventional_treatment_2d, data_df=data_df
        )
