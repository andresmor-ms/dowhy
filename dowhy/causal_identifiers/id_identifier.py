...
class IDIdentifier(CausalIdentifier):
    def __init__(
        ...
        if self.estimand_type != CausalIdentifier.NONPARAMETRIC_ATE:
            raise Exception(
                "The estimand type should be 'non-parametric ate' for the ID method type."
            )
    ...
    def identify_effect(
        ...
        ancestors = find_ancestor(
            outcome_names, node_names, adjacency_matrix, node2idx, idx2node
        )
        if (
        ...
        ancestors = find_ancestor(
            outcome_names, node_names, adjacency_matrix_do_x, node2idx, idx2node
        )
        ...
        adjacency_matrix_minus_x = induced_graph(
            node_set=node_names_minus_x,
            adjacency_matrix=adjacency_matrix,
            node2idx=node2idx,
        )
        c_components = find_c_components(
            adjacency_matrix=adjacency_matrix_minus_x,
            node_set=node_names_minus_x,
            idx2node=idx2node_minus_x,
        )
        ...
        c_components_G = find_c_components(
            adjacency_matrix=adjacency_matrix, node_set=node_names, idx2node=idx2node
        )
