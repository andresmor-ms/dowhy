...
class NodePair:
    ...
    def __init__(self, node1, node2):
        ...
        self._node2 = node2
        self._is_blocked = (
            None  # To store if all paths between node1 and node2 are blocked
        )
        self._condition_vars = (
            []
        )  # To store variable to be conditioned on to block all paths between node1 and node2
        self._complete = (
            False  # To store to paths between node pair have been completely explored.
        )
...
class Path:
    ...
    def __str__(self):
        ...
        if not self._is_blocked:
            string += (
                "To block path, condition on: " + ",".join(self._condition_vars) + "\n"
            )
        return string
...
class Backdoor:
    ...
    def get_backdoor_vars(self):
        ...
        for node1 in self._nodes1:
            for node2 in self._nodes2:
                ...
                if len(path_dict) != 0:
                    obj = HittingSetAlgorithm(
                        path_dict[(node1, node2)].get_condition_vars(), self._colliders
                    )
    ...
    def _path_search_util(
        ...
        if ((node1, node2) in path_dict) and (path_dict[(node1, node2)].is_complete()):
            ...
            if node1 == node2:
                ...
                if (
                    self.is_backdoor(path)
                    and len(self._nodes12.intersection(set(path[1:-1]))) == 0
                ):
                ...
                for neighbour in graph[node1]:
                    if neighbour not in vis:
                        ...
                        next_arrow = (
                            False if self._graph.has_edge(node1, neighbour) else True
                        )
    ...
    def _path_search(self, graph, node1, node2, path_dict):
        ...
        vis = set()
        self._path_search_util(
            graph, node1, node2, vis, [], path_dict, is_blocked=False
        )
