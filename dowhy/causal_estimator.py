...
class CausalEstimator:
        ...
        **kwargs,
        ...
        if self._effect_modifier_names:
            ...
            if len(self._effect_modifier_names) > 0:
                ...
                self.logger.debug(
                    "Effect modifiers: " + ",".join(self._effect_modifier_names)
                )
            else:
    ...
    def estimate_effect_naive(self):
        ...
        est = np.mean(df_withtreatment[self._outcome_name]) - np.mean(
            df_notreatment[self._outcome_name]
        )
    ...
    def _estimate_effect_fn(self, data_df):
        ...
        raise NotImplementedError(
            (
                "Conditional treatment effects are "
                + CausalEstimator.DEFAULT_NOTIMPLEMENTEDERROR_MSG
            ).format(self.__class__)
        )

    def _estimate_conditional_effects(
        self, estimate_effect_fn, effect_modifier_names=None, num_quantiles=None
    ):
        ...
        for i in range(len(effect_modifier_names)):
            ...
            if pd.api.types.is_numeric_dtype(self._data[em].dtypes):
                self._data[prefix + str(em)] = pd.qcut(
                    self._data[em], num_quantiles, duplicates="drop"
                )
    ...
    def _do(self, x, data_df=None):
        raise NotImplementedError(
            (
                "Do-operator is " + CausalEstimator.DEFAULT_NOTIMPLEMENTEDERROR_MSG
            ).format(self.__class__)
        )
    ...
    def construct_symbolic_estimator(self, estimand):
        raise NotImplementedError(
            ("Symbolic estimator string is ").format(self.__class__)
        )

    def _generate_bootstrap_estimates(
        self, num_bootstrap_simulations, sample_size_fraction
    ):
        ...
        if sample_size > len(self._data):
            self.logger.warning(
                "WARN: The sample size is greater than the data being sampled"
            )

        self.logger.info("INFO: The sample size: {}".format(sample_size))
        self.logger.info(
            "INFO: The number of simulations: {}".format(num_bootstrap_simulations)
        )
    ...
    def _estimate_confidence_intervals_with_bootstrap(
        ...
        elif CausalEstimator.is_bootstrap_parameter_changed(
            self._bootstrap_estimates.params, locals()
        ):
        ...
        upper_bound_index = int(
            (1 - confidence_level) * len(sorted_bootstrap_variations)
        )
    ...
    def _estimate_confidence_intervals(
        self, confidence_level=None, method=None, **kwargs
    ):
        """
    ...
    def estimate_confidence_intervals(
        self, estimate_value, confidence_level=None, method=None, **kwargs
    ):
        ...
        if method == "default" or method is True:  # user has not provided any method
            try:
                ...
                confidence_intervals = (
                    self._estimate_confidence_intervals_with_bootstrap(
    ...
    def _estimate_std_error_with_bootstrap(
        self, num_simulations=None, sample_size_fraction=None
    ):
    ...
    def _test_significance_with_bootstrap(
        self, estimate_value, num_null_simulations=None
    ):
        ...
        do_retest = (
            self._bootstrap_null_estimates is None
        ...
        if estimate_value > median_estimate:
            ...
            estimate_index = np.searchsorted(
                sorted_null_estimates, estimate_value, side="left"
            )
            p_value = 1 - (estimate_index / num_null_simulations)
        if estimate_value <= median_estimate:
            # Being conservative with the p-value reported
            estimate_index = np.searchsorted(
                sorted_null_estimates, estimate_value, side="right"
            )
        ...
        if p_value == 0:
            p_value = (
                0,
                1 / len(sorted_null_estimates),
            )  # a tuple determining the range.
        elif p_value == 1:
    ...
    def evaluate_effect_strength(self, estimate):
        fraction_effect_explained = self._evaluate_effect_strength(
            estimate, method="fraction-effect"
        )
    ...
    def _evaluate_effect_strength(self, estimate, method="fraction-effect"):
        ...
        if method not in supported_methods:
            raise NotImplementedError(
                "This method is not supported for evaluating effect strength"
            )
...
class CausalEstimate:
    ...
    def test_stat_significance(self, method=None, **kwargs):
        ...
        signif_results = self.estimator.test_significance(
            self.value, method=method, **kwargs
        )
    ...
    def __str__(self):
        ...
        if hasattr(self, "estimator"):
            ...
            if self.estimator._confidence_intervals:
                s += "{0}% confidence interval: {1}\n".format(
                    100 * self.estimator.confidence_level,
        ...
        if self.effect_strength is not None:
            ...
            s += "Change in outcome attributable to treatment: {}\n".format(
                self.effect_strength["fraction-effect"]
            )
