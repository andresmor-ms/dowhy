...
class DoSampler:
    ...
    def __init__(
        ...
        self.indep_type = [
            ...
            for var in self._treatment_names
    ...
    def point_sample(self):
        if self.num_cores == 1:
            sampled_outcomes = self._df[
                self._treatment_names + self._target_estimand.get_backdoor_variables()
            ].apply(self._sample_point, axis=1)
        else:
            ...
            sampled_outcomes = pd.DataFrame(
                sampled_outcomes, columns=self._outcome_names
            )
    ...
    def _infer_variable_types(self):
        raise NotImplementedError(
            "Variable type inference not implemented. Use the variable_types kwarg."
        )
