...
class TextualEffectInterpreter(TextualInterpreter):
    ...
    def interpret(self):
        ...
        if pd.api.types.is_numeric_dtype(self.estimator._outcome.dtypes):
            # Outcome is numeric
            if all(
                pd.api.types.is_numeric_dtype(tr_dtype)
                or pd.api.types.is_bool_dtype(tr_dtype)
                ...
                raise NotImplementedError(
                    "Interpretation not supported yet for categorical treatments"
                )
        else:
        ...
        interpret_text += (
            ", over the data distribution/population represented by the dataset."
        )
