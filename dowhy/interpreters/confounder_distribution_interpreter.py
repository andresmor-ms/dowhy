...
class ConfounderDistributionInterpreter(VisualInterpreter):
    ...
    def interpret(self):
        ...
        # add weight column
        df.loc[:, "weight"] = df.loc[:, treated] * (propensity) ** (-1) + (
            1 - df.loc[:, treated]
        ) * (1 - propensity) ** (-1)
        ...
        barplot_df_before = (
            df.groupby([self.var_name, treated]).size().reset_index(name="count")
        )

        # after weights are applied we need to sum over the given weights
        barplot_df_after = (
            df.groupby([self.var_name, treated]).agg({"weight": np.sum}).reset_index()
        )
        ...
        iterable = zip(
            [barplot_df_before, barplot_df_after], [ax1, ax2], [title1, title2]
        )
