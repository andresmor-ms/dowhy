...
from dowhy.causal_estimators.propensity_score_stratification_estimator import (
...
class PropensityBalanceInterpreter(VisualInterpreter):
    ...
    def __init__(self, estimate, **kwargs):
        ...
        if not any(
            ...
            for est_class in PropensityBalanceInterpreter.SUPPORTED_ESTIMATORS
        ):
    ...
    def interpret(self):
        ...
        df_long = (
            pd.wide_to_long(
                df.reset_index(), stubnames=["W"], i="index", j="common_cause_id"
            )
            .reset_index()
            .astype({"W": "float64"})
        )
        ...
        mean_diff = df_long.groupby(
            self.estimator._treatment_name + ["common_cause_id", "strata"]
        ).agg(mean_w=("W", np.mean))
        mean_diff = (
            mean_diff.groupby(["common_cause_id", "strata"])
            .transform(lambda x: x.max() - x.min())
            .reset_index()
        )
        mean_diff = mean_diff.query("v0==True")
        size_by_w_strata = (
            df_long.groupby(["common_cause_id", "strata"])
            .agg(size=("propensity_score", np.size))
            .reset_index()
        )
        size_by_strata = (
            df_long.groupby(["common_cause_id"])
            .agg(size=("propensity_score", np.size))
            .reset_index()
        )
        size_by_strata = pd.merge(
            size_by_w_strata, size_by_strata, on="common_cause_id"
        )
        mean_diff_strata = pd.merge(
            mean_diff, size_by_strata, on=("common_cause_id", "strata")
        )
        ...
        mean_diff_strata = pd.merge(
            mean_diff_strata, stddev_by_w_strata, on=["common_cause_id", "strata"]
        )
        ...
        mean_diff_overall = df_long.groupby(
            self.estimator._treatment_name + ["common_cause_id"]
        ).agg(mean_w=("W", np.mean))
        mean_diff_overall = (
            mean_diff_overall.groupby("common_cause_id")
            .transform(lambda x: x.max() - x.min())
            .reset_index()
        )
        mean_diff_overall = mean_diff_overall[
            mean_diff_overall[self.estimator._treatment_name[0]] == True
        ]  # TODO
        stddev_overall = (
            df_long.groupby(["common_cause_id"]).agg(stddev=("W", np.std)).reset_index()
        )
        mean_diff_overall = pd.merge(
            mean_diff_overall, stddev_overall, on=["common_cause_id"]
        )
