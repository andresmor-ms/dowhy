...
class CausalRefuter:
    ...
    def choose_variables(self, required_variables):
        ...
        if required_variables is False:
            ...
            if all(variable[0] == "-" for variable in required_variables):
                ...
                self.logger.error(
                    "{} has both select and delect variables".format(required_variables)
                )
                raise ValueError(
            ...
            self.logger.error(
                "Incorrect type: {}. Expected an int,list or bool".format(
    ...
    def test_significance(
        self, estimate, simulations, test_type="auto", significance_level=0.05
    ):
...
class CausalRefutation:
    ...
    def __str__(self):
        if self.refutation_result is None:
            ...
            return "{0}\nEstimated effect:{1}\nNew effect:{2}\np value:{3}\n".format(
                self.refutation_type,
                self.estimated_effect,
                self.new_effect,
                self.refutation_result["p_value"],
            )
