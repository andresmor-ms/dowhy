...
class CausalGraph:
    ...
    def __init__(
        ...
        if isinstance(graph, str) and re.match(
            r"^dag", graph
        ):  # Convert daggity output to dot format
        ...
        if graph is None:
            ...
            try:
                ...
                self.logger.error(
                    "Pygraphviz cannot be loaded. " + str(e) + "\nTrying pydot..."
                )
                try:
                    import pydot
    ...
    def view_graph(self, layout="dot", size=(8, 6), file_name="causal_model"):
        ...
        try:
            ...
            solid_edges = [
                (n1, n2)
                for n1, n2, e in self._graph.edges(data=True)
                if "style" not in e
            ]
            dashed_edges = [
                (n1, n2)
                for n1, n2, e in self._graph.edges(data=True)
                if ("style" in e and e["style"] == "dashed")
            ]
            plt.clf()
            ...
            nx.draw_networkx_edges(
                self._graph, pos, edgelist=solid_edges, arrowstyle="-|>", arrowsize=12
            )
            nx.draw_networkx_edges(
                self._graph,
                pos,
    ...
    def build_graph(
        ...
        for treatment, outcome in itertools.product(
        ...
        if instrument_names:
            if type(instrument_names[0]) != tuple:
                if len(self.treatment_name) > 1:
                    self.logger.info(
                        "Assuming Instrument points to all treatments! Use tuples for more granularity."
                    )
                for instrument, treatment in itertools.product(
                    instrument_names, self.treatment_name
                ):
    ...
    def get_unconfounded_observed_subgraph(self):
        observed_nodes = [
            ...
            if self._graph.nodes[node]["observed"] == "yes"
        ]
        return self._graph.subgraph(observed_nodes)

    def do_surgery(
        self, node_names, remove_outgoing_edges=False, remove_incoming_edges=False
    ):
    ...
    def check_dseparation(
        self, nodes1, nodes2, nodes3, new_graph=None, dseparation_algo="default"
    ):
        if dseparation_algo == "default":
            if new_graph is None:
                new_graph = self._graph
            dseparated = nx.algorithms.d_separated(
                new_graph, set(nodes1), set(nodes2), set(nodes3)
            )
        else:
            raise ValueError(
                f"{dseparation_algo} method for d-separation not supported."
            )
        return dseparated
    ...
    def get_instruments(self, treatment_nodes, outcome_nodes):
        ...
        g_no_parents_treatment = self.do_surgery(
        ...
        self.logger.debug(
            "Candidate instruments after satisfying exclusion: %s",
        ...
        self.logger.debug(
            "Candidate instruments after satisfying exclusion and as-if-random: %s",
            instruments,
        )
    ...
    def check_valid_frontdoor_set(
        ...
        if dseparation_algo == "default":
            if new_graph is None:
                new_graph = self._graph
            dseparated = nx.algorithms.d_separated(
                new_graph, set(nodes1), set(nodes2), set(candidate_nodes)
            )
        elif dseparation_algo == "naive":
    ...
    def check_valid_mediation_set(
        self, nodes1, nodes2, candidate_nodes, mediation_paths=None
    ):
        ...
        is_mediator = any(
            [self.is_blocked(path, candidate_nodes) for path in mediation_paths]
        )
        return is_mediator
